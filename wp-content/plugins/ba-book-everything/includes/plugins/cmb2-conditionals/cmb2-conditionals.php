 ...
 **/
 // Plugin Name:       CMB2 Conditionals ext
 ...
 //
 // Modified by Sergey Kracvchenko (Booking Algorithms, http://ba-booking.com) to hide group field wich contains conditional field and to use on front end, 2017.
 //
 //
 /**
...
if ( ! class_exists( 'CMB2_Conditionals_ext' ) && ! class_exists( 'CMB2_Conditionals' ) ) {

	/**
	 * CMB2_Conditionals Plugin.
	 */
	class CMB2_Conditionals_ext {
		...
		public function enqueue_scripts() {
            ...
            if ( isset($_GET['inner_page']) && $_GET['inner_page'] == 'edit-post' && isset($_GET['edit_post_id']) ){
	...
	if ( ( function_exists( 'wp_installing' ) && wp_installing() === false ) || ( ! function_exists( 'wp_installing' ) && ( ! defined( 'WP_INSTALLING' ) || WP_INSTALLING === false ) ) ) {
		add_action( 'plugins_loaded', 'cmb2_conditionals_ext_init' );
	}

	if ( ! function_exists( 'cmb2_conditionals_ext_init' ) ) {
		/**
		 * Initialize the class.
		 */
		function cmb2_conditionals_ext_init() {
			$cmb2_conditionals = new CMB2_Conditionals_ext();
		}
	}
