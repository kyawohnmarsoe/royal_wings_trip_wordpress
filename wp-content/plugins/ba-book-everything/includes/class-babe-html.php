...
////////////////////////////
    ...
    public static function get_time_select_arr( $date, $post_id, $is_from = true, $start_time = '00:00', $order_item_args = [], $step = 60, $is_hourly = false ){
        ...
        if ( $is_hourly ){
            ...
            if ( !$is_from && $start_time !== '00:00' ) {
                $start_time_obj->modify('+1 hour');
            }

            // set all times by default
            $output = $times;
            $prev_av_time = '';

            // extend av times with closest not available
            foreach ( $times as $time => $available ){

                if ( !$available ){

                    if ( !$is_from && $prev_av_time ){
                        $output[$time] = 1;
                        $prev_av_time = '';
                    }
                    continue;
            ...
            foreach ( $output as $time => $available ){

                $cur_time_obj = new DateTime( $date_from_obj->format('Y-m-d') . ' ' . $time);

                if ( $cur_time_obj < $start_time_obj ){
                    $output[$time] = 0;
                    continue;
                }
                ...
                for( $i=$start_hour; $i <= 23; $i++ ){
                    ...
                    $i_text = $i < 10 ? '0'.$i : $i;
                    $current_time = $i_text.':00';

                    if( !empty($output[$current_time]) ){

                        $count++;

                        if ( !$is_from && $max_booking_period && $count > $max_booking_period ){

                            for( $j=$i; $j <= 23; $j++ ){
                                $j_text = $j < 10 ? '0'.$j : $j;
                                $current_time_j = $j_text.':00';
                                $output[$current_time_j] = 0;
                            }
                            break;
                        }
                    ...
                    if ( $min_booking_period && $count < $min_booking_period ){

                        for( $j=$start_hour-1; $j < $i; $j++ ){
                            $j_text = $j < 10 ? '0'.$j : $j;
                            $current_time_j = $j_text.':00';
                            $output[$current_time_j] = 0;
                        }
...
////////////////////////////
    ...
    public static function checkout_form($args){
        ...
        $currency = $args['order_currency'] ? $args['order_currency'] : BABE_Order::get_order_currency($order_id);
