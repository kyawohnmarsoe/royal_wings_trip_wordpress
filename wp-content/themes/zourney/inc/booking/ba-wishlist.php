...
if ( ! class_exists( 'Zourney_BA_Booking_Wishlist' ) ) :
	class Zourney_BA_Booking_Wishlist {
		...
		public function create_page_wishlist() {
			$page_wishlist_id = BABE_Install::create_page( 'ba-wishlist', 'ba_wishlist', esc_html__( 'Ba Wishlist', 'zourney' ), '[ba_wishlist]' );
		...
		public function settings_page_wishlist() {
			...
			add_settings_field(
				'wishlist_active', // ID
				__( 'Enable wishlist?', 'zourney' ), // Title
			...
			add_settings_field(
				'wishlist_page', // ID
				__( 'Select wishlist page', 'zourney' ), // Title
			...
			add_settings_field(
				'wishlist_icon', // ID
				__( 'Wishlish icon class', 'zourney' ), // Title
			...
			add_settings_field(
				'wishlist_added', // ID
				__( 'Wishlish added text', 'zourney' ), // Title
		...
		public static function setting_wishlist_active() {
			...
			echo '<p><input id="' . BABE_Settings::$option_name . '[wishlist_active]1" name="' . BABE_Settings::$option_name . '[wishlist_active]" type="radio" value="1" ' . $checked1 . '/><label id="' . BABE_Settings::$option_name . '_wishlist_active1" for="' . BABE_Settings::$option_name . '[wishlist_active]1">' . esc_html__( 'Yes', 'zourney' ) . '</label></p>';
			echo '<p><input id="' . BABE_Settings::$option_name . '[wishlist_active]2" name="' . BABE_Settings::$option_name . '[wishlist_active]" type="radio" value="0" ' . $checked2 . '/><label id="' . BABE_Settings::$option_name . '_wishlist_active2" for="' . BABE_Settings::$option_name . '[wishlist_active]2">' . esc_html__( 'No', 'zourney' ) . '</label></p>';
		...
		public function text_field_callback( $args ) {
			...
			printf(
				...
				isset( BABE_Settings::$settings[ $args['option'] ] ) ? esc_attr( BABE_Settings::$settings[ $args['option'] ] ) : 'zourney-icon-heart'
			);
		}
		...
		public static function add_to_wishlist( $book_id ) {
			...
			if ( is_user_logged_in() ) {
				...
				$text           = ( $is_in_wishlist ) ? esc_html__( 'View list Wishlist', 'zourney' ) : esc_html__( 'Add to wishlist', 'zourney' );
			} else {
				$wishlist_link = '#zourney-login-form';
				$class         = 'login-acount';
				$text          = esc_html__( 'Add to wishlist', 'zourney' );
			}

			// Hook for icon HTML
			$icon_class = isset( BABE_Settings::$settings['wishlist_icon'] ) && !empty(BABE_Settings::$settings['wishlist_icon']) ? BABE_Settings::$settings['wishlist_icon'] : 'zourney-icon-heart';
		...
		public function ajax_update_wishlist() {
			...
			if ( $result ) {
				$added_text         = isset( BABE_Settings::$settings['wishlist_added'] ) ? BABE_Settings::$settings['wishlist_added'] : esc_html__( 'Product Added!', 'zourney' );
				$data['added_text'] = $added_text;

				$data['wishlist_text'] = esc_html__( 'View list Wishlist', 'zourney' );
		...
		public function wishlist_enqueue_scripts() {
			...
			wp_enqueue_script( 'zourney-wishlist', get_theme_file_uri( '/assets/js/frontend/wishlist.js' ), [], $zourney_version );
...
return new Zourney_BA_Booking_Wishlist();
