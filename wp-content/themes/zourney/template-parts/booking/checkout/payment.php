...
$deposit = '';
if ($payment_model == 'deposit_full') $deposit = esc_html__('(deposit)', 'zourney');
...
if ($action == 'to_pay') {
    ...
    <div class="amount_group">
        <label class="checkout_form_amount_label"><?php esc_html_e('Amount to Pay now:', 'zourney') ?></label>
            ...
            <div class="checkout_form_pay_total">
                <div class="input-cicrle">
                    ...
                    <label for="order_amount_to_pay_full">
                        <?php echo ent2ncr(BABE_Currency::get_currency_price($total_amount)) . '&nbsp;' . esc_html__('(full)', 'zourney'); ?>
                    </label>
                </div>
            </div>
        <?php } ?>
    </div>
    <h2 class="checkout-title"><?php esc_html_e('Payment Method', 'zourney') ?></h2>
    ...
    <h2><?php esc_html_e('Payment details', 'zourney') ?></h2>
    <div class="payment_group payment_details_before_av_check">
        <?php sprintf(esc_html__('You could pay after the items availability confirmation! <br> Confirmation will be sent to your e-mail. <br> Amount to pay online will be %s.', 'zourney'), BABE_Currency::get_currency_price($prepaid_amount)); ?>
    </div>
    <?php
}
